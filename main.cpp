...
{
    if (b.is_even()) {	                                             // Check if b is even
        b += 1;                                                      // If even make b odd by adding one
        int ld = b.get_last_digit();
...
int main(int argc, char** argv)
{
    ...
    std::vector<prime> primes; prime ini_p(ini3, 0); primes.push_back(ini_p);
    ...
    for (int z=0; z<iter; z++) {
        ...
        for (int print_st=prime_print_ind; print_st < primes.size(); print_st++)
            std::cout << ((primes.at(print_st)).get_bigint()).to_string() << std::endl;
